...
| 运行时 SDK | ~~**`@lark-base-open/js-sdk`**~~ → **`@lark-opdev/block-bitable-api`** | ⚠️ **原选型错误，已裁定迁移**（2026-09-20，详见 `03` §0.3）。原理由"文档最全、能力较多"是**错误依据**——真正依据只能是**插件形态对应的官方开发指南指定哪个 SDK**：我们是「多维表格**数据表视图**」（Pack/block），官方《多维表格数据表视图开发指南》明确指定 `@lark-opdev/block-bitable-api`；`js-sdk` 是给「多维表格**扩展脚本**」用的。两者**协议互不兼容**（handshake vs iframe-block），真机实测宿主实现的是 iframe-block → 旧 SDK 全部 API 统一 `time out` | `@lark-base-open/js-sdk`（❌ 协议不匹配，已排除） | **强制**（按官方对应形态的开发指南） |
...
5. **⚠️ 上传成功 ≠ 版本已生效**：见 **§13.5.1**（本轮真实事故，排查方向曾被带偏）。

### 13.5.1 ⚠️ 上传 ≠ 生效：版本切换是独立且**每次都必须重复**的步骤（2026-09-20 事故修正）

> **标注**：以下"事故经过"为**【实测】**；"根因"中"upload 不切换版本"为**【实测】**（已由官方流程与现象双重印证）。

**事故经过**【实测】：
- 用户浏览器实际加载的一直是 **09:38** 的旧产物（`index.d39a0c4…js`）；
- 我们 **10:12** 上传的新版本**从未生效**；
- 于是"修复后问题依旧"被**误读为"修复无效"**，排查方向被带偏，额外消耗了一轮定位成本。

**根因**【实测】：`opdev upload` **只上传代码物料**，**不会**切换开发者后台里线上/调试环境实际使用的版本。
→ **§13.5 流程图中的 B（后台选版本号 + 上传图标 + 填名称/介绍 + 保存）不是"首次发布才做"的一次性步骤，而是每次 `upload` 之后都要重做**。

**官方文档要求（原文）**：
> 重新进入开发者后台，切换到**数据表视图**。「小组件版本」选刚上传时填入的版本号，然后上传图标、填写名称和介绍。最后点击**保存**。

**修订后的必做步骤（每次 `opdev upload` 之后）**：
1. 开发者后台 → 切换到 **数据表视图**；
2. 「**小组件版本**」→ 选择**刚上传时填写的那个版本号**；
3. 上传图标、填写名称与介绍；
4. 点击 **保存**。

> **缺任一步 → 线上仍然跑旧版本**，且 CLI 不会给出任何提示。

**判断"新版本是否真的生效"的可靠方法**：

> **比对浏览器控制台报错信息右侧显示的 chunk 文件名（带 hash），与本地 `dist/` 中实际生成的文件名是否一致。**

- 依据【实测】：`webpack.config.js` 输出 `filename: 'index.[contenthash].js'` → **内容一变，hash 必变**，文件名天然是版本指纹；
- **一致** = 新版本已生效；**不一致** = 线上仍是旧包 —— 此时任何"修复无效"的结论都**不成立**；
- 补充核验点：DevTools Network 中该 js 的 **体积 / Last-Modified**，与本地 `dist/index.*.js` 交叉比对；
- ❌ **不要**仅凭 CLI 输出"上传成功"就认为已生效。

**纪律（务必遵守）**：
出现"改了但现象没变"时，**第一步先做上述文件名比对**，确认版本已生效，再讨论代码。
**禁止**在未确认版本生效的前提下，把现象解读为"修复无效 / 方案错误 / 方向不对"。

> 本条与 `03-开发设计文档.md` §0.2.1 属**同一类返工来源**：把**未经核实的前提**（版本已生效 / 推断已成立）当成结论使用。今后凡"现象与预期不符"，先核查前提，再质疑方案。
...
- [ ] **已在开发者后台把「小组件版本」切到刚上传的版本号并保存**（§13.5.1）
- [ ] **已比对控制台 chunk 文件名 hash 与本地 `dist/` 实际文件名一致**（§13.5.1，证明新版本真的生效）
...
|---|---|---|---|---|
| `@lark-opdev/block-bitable-api` | `0.4.1-alpha.11`（**精确锁定**，该包无 stable tag） | 官方《多维表格数据表视图开发指南》指定的运行时 SDK（base/bridge/ui/dashboard/util） | ✅ | **官方指定**（按插件形态） |
| ~~`@lark-base-open/js-sdk`~~ | ~~`^1.0.0`~~ | ~~扩展脚本 SDK~~ → **协议不匹配（handshake vs iframe-block），已裁定移除**（`03` §0.3） | ❌ | 误选，勿再用 |
